...
export const ExperienceForm: React.FC = () => {
  ...
  const [expandedIds, setExpandedIds] = useState<Record<string, boolean>>(() => {
    ...
    if (experienceList.length > 0) {
      initial[experienceList[0].identifier] = true;
    }
    return initial;
  });
  ...
  const handleApplyFastPaste = (expId: string) => {
    if (!fastPasteState || fastPasteState.id !== expId) return;
    ...
    if (lines.length > 0) {
      updateExperience(expId, 'bulletPoints', lines);
    }
  ...
  const handleAddNew = () => {
    ...
    addExperience({ identifier: newId });
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      >
        ...
        {translate(lang, 'experience.add')}
      </button>

      {/* Experience Cards List */}
      <div className="flex flex-col gap-3.5">
        {experienceList.map((exp, index) => {
          const isExpanded = expandedIds[exp.identifier] ?? (index === 0);
          const isFastPasting = fastPasteState?.id === exp.identifier;
          const displayTitle = (exp.jobTitle || '').trim() || translate(lang, 'experience.role');
          const displayCompany = (exp.companyName || '').trim() || translate(lang, 'experience.company');
          const dateRange = [exp.startDate, exp.isCurrentRole ? (lang === 'fr' ? 'Présent' : 'Present') : exp.endDate]
            .filter(Boolean)
            .join(' – ');
          ...
          return (
            ...
            >
              ...
              <div
                ...
                onClick={() => toggleExpand(exp.identifier)}
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={() => removeExperience(exp.identifier)}
                      title={translate(lang, 'experience.remove')}
                ...
                <div className="flex items-center gap-2 text-[11px] text-slate-500 dark:text-slate-400 pl-7">
                  <span className="font-medium truncate">@ {displayCompany}</span>
                ...
                <div className="p-4 pt-3 border-t border-slate-100 dark:border-slate-800 flex flex-col gap-3.5">
                  ...
                  <div>
                    <label className="text-[11px] font-semibold text-slate-700 dark:text-slate-300 mb-1 flex items-center gap-1.5">
                      ...
                      {translate(lang, 'experience.role')}
                    </label>
                    <input
                      type="text"
                      value={exp.jobTitle}
                      onChange={(e) => updateExperience(exp.identifier, 'jobTitle', e.target.value)}
                  ...
                  <div>
                    <label className="text-[11px] font-semibold text-slate-700 dark:text-slate-300 mb-1 flex items-center gap-1.5">
                      ...
                      {translate(lang, 'experience.company')}
                    </label>
                    <input
                      type="text"
                      value={exp.companyName}
                      onChange={(e) => updateExperience(exp.identifier, 'companyName', e.target.value)}
                  ...
                  <div>
                    <label className="text-[11px] font-semibold text-slate-700 dark:text-slate-300 mb-1 flex items-center gap-1.5">
                      ...
                      {translate(lang, 'experience.location')}
                    </label>
                    <input
                      type="text"
                      value={exp.locationName || ''}
                      onChange={(e) => updateExperience(exp.identifier, 'locationName', e.target.value)}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-[11px] font-semibold text-slate-700 dark:text-slate-300 mb-1 flex items-center gap-1.5">
                        ...
                        {translate(lang, 'experience.startDate')}
                      </label>
                      <input
                        type="text"
                        value={exp.startDate}
                        onChange={(e) => updateExperience(exp.identifier, 'startDate', e.target.value)}
                    ...
                    <div>
                      <label className="text-[11px] font-semibold text-slate-700 dark:text-slate-300 mb-1 flex items-center gap-1.5">
                        <Calendar className="w-3.5 h-3.5 text-slate-400" />
                        {translate(lang, 'experience.endDate')}
                      </label>
                      <input
                        type="text"
                        disabled={exp.isCurrentRole}
                        value={exp.isCurrentRole ? (lang === 'fr' ? 'Présent' : 'Present') : exp.endDate}
                        onChange={(e) => updateExperience(exp.identifier, 'endDate', e.target.value)}
                        placeholder="e.g. Present"
                        className="w-full px-3 py-2 text-xs rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 placeholder-slate-400 disabled:bg-slate-100 dark:disabled:bg-slate-800 disabled:text-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500 transition-all"
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-2.5 pt-2 border-t border-slate-100 dark:border-slate-800">
                    <div className="flex items-center justify-between flex-wrap gap-2">
                      <label className="text-[11px] font-bold text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
                        ...
                        {translate(lang, 'experience.bullets')}
                      </label>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        >
                          ...
                          <span>{translate(lang, 'experience.fastPaste')}</span>
                        </button>
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            updateExperience(exp.identifier, 'bulletPoints', nextBullets);
                          }}
                        ...
                        >
                          ...
                          <span>{translate(lang, 'experience.addBullet')}</span>
                        </button>
                      </div>
                    </div>
                    ...
                    {isFastPasting ? (
                      <div className="p-3.5 rounded-xl border border-blue-200 dark:border-blue-800 bg-blue-50/50 dark:bg-blue-950/30 flex flex-col gap-2.5">
                        <div className="text-[11px] font-medium text-blue-800 dark:text-blue-300">
                          {translate(lang, 'experience.fastPasteHelp')}
                        </div>
                        <textarea
                          rows={4}
                          value={fastPasteState.text}
                          onChange={(e) =>
                            setFastPasteState({ id: exp.identifier, text: e.target.value })
                          }
                          placeholder="Architected cloud migration strategy delivering 99.99% availability&#10;Spearheaded cross-functional delivery team across 4 domains&#10;Reduced latency by 45% through aggressive query optimization"
                        ...
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            type="button"
                            onClick={() => handleApplyFastPaste(exp.identifier)}
                          ...
                          >
                            ...
                            {translate(lang, 'experience.applyBullets')}
                          </button>
                        </div>
                      </div>
                    ) : (
                        ...
                        {(exp.bulletPoints || []).map((bullet, bIdx) => (
                          ...
                          >
                            ...
                            <textarea
                              ...
                              placeholder={translate(lang, 'experience.bulletPlaceholder')}
                            ...
                            <div className="flex flex-col items-center gap-0.5 shrink-0 pt-0.5">
                              <button
                                type="button"
                                disabled={bIdx === 0}
                                onClick={() => reorderExperienceBullets(exp.identifier, bIdx, bIdx - 1)}
                              ...
                              <button
                                type="button"
                                disabled={bIdx === (exp.bulletPoints?.length || 1) - 1}
                                onClick={() => reorderExperienceBullets(exp.identifier, bIdx, bIdx + 1)}
                              ...
                              <button
                                type="button"
                                onClick={() => {
                                  const nextBullets = (exp.bulletPoints || []).filter((_, i) => i !== bIdx);
                                  updateExperience(exp.identifier, 'bulletPoints', nextBullets);
                                }}
                                title="Remove Bullet"
